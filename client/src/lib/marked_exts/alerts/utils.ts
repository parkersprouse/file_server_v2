 ...
 */
const default_alert_variant: AlertVariantItem[] = [
  {
...
export function resolveVariants(variants: AlertVariantItem[]): AlertVariantItem[] {
  if (variants.length === 0) return default_alert_variant;

  return Object.values([...default_alert_variant, ...variants].reduce(
    (map, item) => {
      map[item.type] = item;
      return map;
    },
