...
> **✅ Resolved (2026-06-25):** verified the highlighter tokenizes the fetched
> text into the DOM (it does not inject raw HTML), so the preview path escapes
> content. As the recommended hardening, text files are served as `text/plain`
> (already the case) **and** now carry `X-Content-Type-Options: nosniff` (added
> in `server/src/main.rs`), so a `.txt` containing HTML can't be sniffed into an
> executable type — verified against a live response.
>
> **Update (2026-06-29):** highlighting migrated from Prism to **Shiki**
> (`client/src/lib/highlighter/`). The escaping property is unchanged — Shiki
> HTML-escapes all code text (verified in the generated output), `TextPreview.vue`
> injects it via `v-html`, and the only injected markup is generated by us:
> color-swatch `<span>`s and `<a>` autolinks whose `href` is gated to `https?:`
> URLs (no `javascript:` / `data:` schemes can match), serialized with attribute
> escaping. Unlike the Markdown path, no DOMPurify pass is needed because the
> input to `v-html` is highlighter-generated, not arbitrary file HTML.

The highlighter fetches the raw file and renders escaped, tokenized HTML into
`<pre><code>`. This is safe as long as the highlighter escapes text content and
the file is served with a non-executable content type.
...
- **Lazy-loaded preview components and the Shiki highlighter** keep the initial
  bundle small; `manualChunks` splits vendors sensibly. (Shiki replaced Prism on
  2026-06-29 — fine-grained core + JS engine with one code-split chunk per
  grammar, loaded on demand, so the highlighter never ships the full language set
  up front.)
