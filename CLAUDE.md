...
**Fresh-clone gotcha:** the client build fails until `client/src/config.toml` exists — it is gitignored, so copy it from `client/src/config.template.toml` (set `server_port` to wherever the server runs) before `pnpm dev`/`pnpm build`.
