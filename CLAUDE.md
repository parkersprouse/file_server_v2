...
- **Syntax highlighting is Shiki**, lazily loaded from `lib/highlighter/` only when a text preview opens. It uses the fine-grained `shiki/core` build with the **JavaScript RegExp engine** (`shiki/engine/javascript`, no Oniguruma WASM) and loads each grammar on demand from `bundledLanguages` — so a preview fetches only the highlighter core chunk + the one grammar chunk it needs (never the whole language set). `languages.ts` maps a file extension → Shiki language id (plaintext fallback). Output is **dual-theme** (`one-light` / `one-dark-pro`, `defaultColor: false`), so light/dark and the wrap / inline-color toggles are **pure CSS** (`_highlighter.css`, class-based `.dark`) with **no re-highlighting** — line numbers are CSS counters too. Two Prism-parity features live in `transformers.ts` as custom Shiki transformers: inline CSS **color swatches** (sets the store's `inline_colors_present`) and http(s) **URL autolinking**. `TextPreview.vue` fetches the raw file once, injects Shiki's HTML via `v-html` (safe: Shiki escapes all code text and links are gated to `https?:`), and publishes `file_highlight_result` (language + inline-colors) which gates the source-only action bar.
